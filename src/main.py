...
from LineSegmentation import line_segmentation
...
def load_different_image(line_count):
    imgs = []
    for i in range(line_count, Model.batchSize):
...
def infer(model, fnImg):
    ...
    line_count = line_segmentation(fnImg)
    imgs = []
    for i in range(line_count-1):
        imgs.append(preprocessor(cv2.imread("../src/static/segment ({}).jpg".format(i+1), cv2.IMREAD_GRAYSCALE), Model.imgSize))

    imgs_test = load_different_image(line_count)
    imgs = imgs + imgs_test
    batch = Batch(None, imgs)
    recognized = model.inferBatch(batch)  # recognize text
    page = []
    for i in range(line_count-1):
       page.append(correct_sentence(recognized[i])) 
